...
Eigen::MatrixXd convertWaypointsToCarPerspective(double carX, double carY, double carPsi, const vector<double> & ptsx, const vector<double> & ptsy){
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
		  ...
		  v *= 0.44704;
		  //Convert ptsx and ptsy to be from the reference point of the car
		  auto waypointsFromCar = convertWaypointsToCarPerspective(px, py, psi, ptsx, ptsy);
          double steer_value;
		  ...
		  Eigen::VectorXd carWaypointsY = waypointsFromCar.row(1);
		  
		  auto coeffs = polyfit(carWaypointsX, carWaypointsY, 3);
		  ...
		  for (unsigned int i = 0; i < solved.size(); i+=2) {
		  ...
		  for (unsigned int i=0; i < ptsx.size(); ++i) {
        	  next_x_vals.push_back(carWaypointsX(i));
        	  next_y_vals.push_back(carWaypointsY(i));
          }
